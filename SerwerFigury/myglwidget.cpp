...
void MyGLWidget::paintGL()
{
    ...
    // poziomy.rysuj(0);
       ...
       // lista odcinkow rysowana w initializeGL - nie trybi
...
void MyGLWidget::zmienPoziom(int i)
{
    usunFigurySlot();
    usunOdicnki();
    listaFigur = poziomy.inicjalizuj(i,ramka,listaOdcinkow);
    aktualnyPoziom = i;
...
void MyGLWidget::animacjaPrzyblizania()
{
    ...
    if( j && wartoscOrtho > ramka)
    {
        wartoscOrtho -= 10;

        if( wartoscOrtho < ramka+11)
        {
            j = 0;
...
void MyGLWidget::ruchFigur()
{
    for(int i=0; i<listaFigur.length(); i++)
    {
        ...
        //        }
                ...
                // jesli wyjdzie za pole renderingu to zostanie usunięta
